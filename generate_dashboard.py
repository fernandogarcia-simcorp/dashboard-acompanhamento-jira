...
.tasks-expanded .task-hidden{display:flex}
.task-toggle{margin-top:11px;background:none;border:1px solid var(--border);border-radius:7px;padding:6px 13px;font-family:inherit;font-size:11.5px;font-weight:700;color:var(--blue);cursor:pointer;letter-spacing:.02em;transition:background .15s,border-color .15s}
.task-toggle:hover{background:var(--gray-50);border-color:var(--blue)}
...
function taskRows(arr,host,tag,kind){host.innerHTML='';host.classList.remove('tasks-expanded');if(!arr.length){host.innerHTML='<p style="color:var(--gray-700);font-size:13px">Nenhum item nesta categoria.</p>';return;}
  ...
  sorted.forEach((t,idx)=>{const c=el('div','task'+(idx>=LIM?' task-hidden':''));
  ...
  if(sorted.length>LIM){const b=el('button','task-toggle');b.type='button';const lbl=()=>host.classList.contains('tasks-expanded')?'ver menos':`ver todos (${sorted.length})`;b.textContent=lbl();b.onclick=()=>{host.classList.toggle('tasks-expanded');b.textContent=lbl();};host.appendChild(b);}}
